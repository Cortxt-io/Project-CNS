...
- **MCP-router (två routrar, skilj dem åt):** (1) `.mcp.json` (versionerad) listar servrarna **externa** klienter (Claude Code mot Railway) når — i dag bara `project-cns`. (2) `scripts/mcp_router.py` + `config/mcp_servers.json` är routern för **agenturens egna lokala pass**: `resolve(role_tools)` monterar per pass de MCP-servrar + verktyg rollens `## Tillåtna verktyg` kräver (baseline `cns`/`web` alltid; externa som `github` per roll, gatade på env, fail-open). Seamet är rollens verktygslista — inte nodfilformen. En separat gateway-process är inritad som noden `mcp-gateway` (`depends_on: cns-mcp`) och byggs först när Plan B-agenter når många servrar / behöver central auth. Beslut + env: `decisions/mcp-router.md`.
