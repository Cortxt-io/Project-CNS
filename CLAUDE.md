...
- `scripts/tui/` — interaktiv terminal-överblick (textual). **Isolerad:** konsumerar bara datalagret (`read_all_nodes`), rör inte `cns.py`. **Control Tower** (epic #8): `OverviewScreen` öppnas vid start (landningsvy, nodträdet bakom esc/o) och visar fyra block ur `data.cockpit_state()`; **"Härnäst" är handlingsbar** (#43, dra-loop idea-5132a8f6) — en OptionList där Enter drar in dig i det rekommenderade passet (`session_store.set_active`+`start_session`+`set_focus`). Körs via `python -m scripts.tui` eller `cns tui` (lazy import — `textual` laddas inte vid varje `cns`-anrop). Beroende: `textual>=0.79,<1.0`.
