...
- `scripts/triage.py` — **idé-triage-grupperare** (#39, Control Tower form a): `group_ideas()` sorterar öppna idéer i åtgärdbara hinkar (kluster per slug · mature · stale · untriaged), transport-fri/testbar så MCP-form (b) och TUI-form (c) återanvänder samma logik. Exponerad som `cns triage [--json]`; driver `.claude/skills/idea-triage.md`.
