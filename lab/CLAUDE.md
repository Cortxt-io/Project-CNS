...
- `app/server.py` — Flask-backend (Railway). **`/api/command-center`** (läs-only, ingen auth, ingen `git_pull`) exponerar `command_center_state()` — den sammanvävda lägesbilden (`{missions, sitrep, logistics, orders, command, freshness}`) — så vilken webyta som helst (idag `cortxt/apps/app` Cockpit) når kompositören, inte bara TUI:t.
