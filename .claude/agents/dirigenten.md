...
Använd PowerShell-verktyget (Bash):
```
Start-Process wt -ArgumentList '-w 0 nt -d "C:\Users\RikardAndersson\CNS projekt\Project-CNS" powershell -NoExit -ExecutionPolicy Bypass -Command claude'
```

**OBS:** Du kan öppna fliken men inte skicka startmeddelandet in i den — det är en begränsning. Skriv istället startmeddelandet i din egen output så Rikard ser det och kan klistra in det i den nya fliken.
