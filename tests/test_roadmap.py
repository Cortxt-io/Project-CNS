...
def test_recipe_phases() -> None:
    ...
    assert keys == ["spec", "mvp", "live", "users", "validated", "paying"]


def test_summary_crusade() -> None:
    s = roadmap.roadmap_summary("crusade")
    assert s is not None
    assert s["current_phase"] == "spec" and s["phase_index"] == 1 and s["total_phases"] == 6
...
def test_detail_merges_recipe_and_status() -> None:
    ...
    assert [p["key"] for p in d["phases"]] == ["spec", "mvp", "live", "users", "validated", "paying"]
