...
_MATRIS = {
    "cells": {
        ...
        "Product|lead": {"tool_families": ["ideas", "issues", "quests"]},
        "Leadership|exec": {"tool_families": ["sessions", "quests", "projects"]},
    }
}


def test_derive_level():
    assert tf.derive_level({"department": "Leadership"}) == "exec"
...
def test_effective_tools_dedupes():
    ...
    role = {"department": "Product", "lead": True, "tools_override": ["issues", "ideas", "cortxt_capture_idea"]}
    out = tf.effective_tools(role, matris=_MATRIS)
...
def test_exec_uses_leadership_cell():
    role = {"department": "Leadership", "tools_override": []}
